...
<div class="map-section">
    ...
    <div class="row">
        ...
        <label style="margin-top: 10px; font-size: 16px;">Search Festivals : </label>
        </div>
        <form action="{{ url('search_festivals') }}" method="post" id="searchFilmMaker">
        <div class="col-md-3" >            
            <input type="text" class="form-control" name="film_maker" placeholder="Festival Name">
        </div>
        <div class="col-md-3">  

            <select class="form-control" name="country">
                <option value="0">Choose A Country</option>
                @foreach(countriesForFestivalMap() as $count => $num)
                   <option value="{{ $count }}">{{ $count.' '.$num }}</option>
                 @endforeach
            </select>
...
<script>
  var ajax_url = '{{ url('festival/num_festivals') }}';
</script>
<script src="{{ asset('site') }}/js/lib/vendors/jqvmap/jqvmap/maps/jquery.vmap.js"></script>
<script src="{{ asset('site') }}/js/lib/vendors/jqvmap/jqvmap/jvmap_festivals.js"></script>
...
<script>
  ...
  @if(count(\App\Festival::all()) > 0)
  @foreach(\App\Festival::orderBy('id','asc')->get() as $film_maker)
  @if($film_maker->country != '')
      {
          @if(file_exists('images/festivals/'.$film_maker->logo_url))
          profilePic: '{{ url('/') }}/images/festivals/{{$film_maker->logo_url}}',
         @else
         ...
         @endif           
          ...
          country_code: '{{ \App\Country::where('name',$film_maker->country)->first()->code }}',
