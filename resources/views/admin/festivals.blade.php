...
@section('content')
			...
			<div class="page-bar">
				<ul class="page-breadcrumb">
					...
					<li>
						<a href="{{ url('admin/festivals') }}">Festivals</a>
					</li>
				</ul>
			</div>
			<h3 class="page-title">
			Festivals <small>reports & statistics</small>
			</h3>
			<!-- END PAGE HEADER-->
				...
				<div class="col-md-12">
					...
					<div class="portlet box grey-cascade">
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-globe"></i>Festivals Table
							</div>
							<div class="tools">
						...
						<div class="portlet-body">
							<div class="table-toolbar">
								<div class="row">
									<div class="col-md-6">
										<div class="btn-group">
											<a type="button" href="{{ url('admin/addfestival') }}" id="sample_editable_1_new" class="btn green">
											Add New Festival <i class="fa fa-plus"></i>
											</a>
										</div>
									</div>
									<div class="col-md-6">
							...
							@foreach(\App\User::showFestivals() as $festival)
							<tr class="odd gradeX"  id="product{{$festival->id}}"><th></th>
								<td>
								 {{ $festival->fullname }}
								</td>
								<td>
									@if(file_exists('images/festivals/'.$festival->logo_url))
									<img src="{{ url('images/festivals').'/'.$festival->logo_url }}" width="50" height="30" alt=""/>
									@else
								...
								<td>
									...
									{!! str_replace(' ', '<br />', ucfirst($festival->country)) !!}
								</td>
								<td class="center">
									 <!-- {{  \Carbon\Carbon::createFromTimeStamp(strtotime($festival->created_at))->diffForHumans() }} -->
									 {{  \Carbon\Carbon::createFromTimeStamp(strtotime($festival->created_at)) }}
								</td>
								<td class="text-center">
								<a href="{{ url('admin/editfestival').'/'.$festival->id }}" class="btn btn-sm yellow"> Edit <i class="fa fa-edit"></i></a><br>

								<a href="javascript:;" data-id="{{ $festival->id }}" class="btn btn-sm red delete-product"> Delete <i class="fa fa-close"></i></a>
								</td>
							</tr>
							@endforeach
        ...
        */
        ...
        var product_id_delete    = $('#product_id_delete').val();
         ...
         $.ajax({
                        ...
                        url:'/admin/deletefestival/'+product_id_delete,
                        type:'post',
                        beforeSend:function(){
