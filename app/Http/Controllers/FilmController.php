    ...
    */
    ...
    public function filterFestival(Request $request)
    {
        ...
        if ($fees == 'no') {
            ...
            $festivalsopen= $festivalsopen->where('festival_competetions.fees',0);
        ...
        if ($duration_time != '0') {
            ...
            $festivalsopen= $festivalsopen->where('festival_competetions.max_duration', '<=', $duration_time);

        }

        if ($country != 'cat') {

            $festivals_coutries = Festival::where('country',$country)
                ->pluck('id')
                ->toArray();


            $festivalsopen = $festivalsopen->whereIn('festivals.id',$festivals_coutries);
        ...
        if ($film_category != 'cate') {
            ...
            $festivalsopen= $festivalsopen->whereIn('festival_competetions.id',$comp_all);

        }
        $film = Session::get('my_film');

                $festivalsopen=$festivalsopen->get();
        ...
        $festivals= new Paginator($currentPageSearchResults, count($collection), $perPage);

        //CRY OUT LOUD

        $data = view('partials.fest_teaser',compact('festivals','festivals_coutries'))->render();
    ...
    public function submitAfilm(Request $request)
    {
       ...
       if (Session::has('my_film')) {
        ...
        if ($film->film_languages != "") {
            ...
            $arr = array_intersect($comp_country,$prod_country);
//&& $film->film_languages != "No Dialogue"
            if (in_array('All Languages',$comp_country) == false && count($arr) == 0 && !(in_array('No Dialogue',$prod_country)) ) {
        ...
        if ($film->subtitles_languages != "") {
            ...
            $arr = array_intersect($comp_country,$prod_country);
//&& $film->subtitles_languages != "No Dialogue"
            if (in_array('All Languages',$comp_country) == false && count($arr) == 0 && !(in_array('No Dialogue',$prod_country))) {
...
//           }
        ...
        /* start check film themes and genres together  */
           $genres = [];
              foreach ($film->genres as $key) {
               $genres[]= $key->name;
           }
        ...
        if (count($themes) == 0 || count($genres) == 0) {
            $comp_genres = json_decode($comp->film_genres);
            $comp_themes = json_decode($comp->film_themes);

            if (in_array('all',$comp_genres) == true){
            ...
            if (in_array('all',$comp_themes) == true){
                if (in_array('all',$comp_genres) == false) {
            ...
            if (in_array('all',$comp_themes) == false && in_array('all',$comp_genres) == false) {

            $reasons[] = 'themes_genres';
        ...
        if (count($themes) > 0 || count($genres) > 0) {
            ...
            if ((in_array('all',$comp_themes) == false && count($themes_arr) == 0) && (in_array('all',$comp_genres) == false && count($genres_arr) == 0)) {

            $reasons[] = 'themes_genres';

            }
            if (in_array('all',$comp_genres) == true && count($genres_arr) == 0){
            ...
            if (in_array('all',$comp_themes) == true && count($themes_arr) == 0){
               if (in_array('all',$comp_genres) == false && count($genres_arr) == 0) {
